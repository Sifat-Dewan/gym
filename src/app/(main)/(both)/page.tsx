import { Footer } from "@/components/footer";
import MaxWidthWrapper from "@/components/max-width-wrapper";
import { buttonVariants } from "@/components/ui/button";
import { InstituteName } from "@/lib/constants";
import { currentUser } from "@/lib/current-user";
import { isModerator } from "@/lib/utils";
import { ArrowRight } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
export const dynamic = "force-dynamic";

export default async function Home() {
  const user = await currentUser();
  return (
    <main className="h-full flex flex-col">
      <MaxWidthWrapper className="flex-grow">
        <div className="mb-12 mt-14 flex flex-col items-center justify-center text-center">
          <div className="px-6 py-2.5 font-semibold border shadow-md rounded-full">
            Welcome to <span className="text-primary">{InstituteName}</span>
          </div>
          <h1 className="max-w-4xl mt-3 text-5xl font-bold md:text-6xl lg:text-7xl">
            Maximize Gains at Our{" "}
            <span className="text-primary">Cutting-Edge </span> Fitness Hub.
          </h1>
          <p className="mt-5 max-w-prose text-foreground/70 sm:text-lg">
            Discover peak fitness with state-of-the-art equipment, expert
            guidance, and a supportive community at our premium gym. Elevate
            your training experience today.
          </p>

          <Link
            className={buttonVariants({
              variant: "gradiant",
              size: "lg",
              className: "mt-5 group",
            })}
            href={isModerator(user) ? "/admin/dashboard" : "/dashboard"}
          >
            Get started{" "}
            <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-full transition" />
          </Link>
        </div>
        <div>
          <div className="relative isolate">
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 -top-40 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-80"
            >
              <div
                style={{
                  clipPath:
                    "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
                }}
                className="relative left-[calc(50%-11rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30 sm:left-[calc(50%-30rem)] sm:w-[72.1875rem]"
              />
            </div>

            <div>
              <div className="mx-auto max-w-6xl px-6 lg:px-8">
                <div className="mt-16 flow-root sm:mt-24">
                  <div className="-m-2 rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4">
                    <Image
                      src="/images/gym_1.jpg"
                      alt="product preview"
                      width={1364}
                      height={866}
                      quality={100}
                      className="rounded-md bg-white p-2 sm:p-8 md:p-20 shadow-2xl ring-1 ring-gray-900/10"
                    />
                  </div>
                </div>
              </div>
            </div>

            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 -top-40 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-80"
            >
              <div
                style={{
                  clipPath:
                    "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
                }}
                className="relative left-[calc(50%-13rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30 sm:left-[calc(50%-36rem)] sm:w-[72.1875rem]"
              />
            </div>
          </div>
        </div>
        <div className="mx-auto mb-32 mt-32 max-w-5xl sm:mt-56">
          <div className="mb-12 px-6 lg:px-8">
            <div className="mx-auto max-w-2xl sm:text-center">
              <h2 className="mt-2 font-bold text-4xl sm:text-5xl">
                Get ready to start sweating in minutes!
              </h2>
              <p className="mt-4 text-lg text-foreground/70">
                Transforming your workout with {InstituteName}: Gym-friendly PDF
                chats!
              </p>
            </div>
          </div>

          {/* steps */}
          <ol className="my-8 space-y-4 pt-8 md:flex md:space-x-12 md:space-y-0 px-4">
            <li className="md:flex-1">
              <div className="flex flex-col space-y-2 border-l-4 border-zinc-300 py-2 pl-4 md:border-l-0 md:border-t-2 md:pb-0 md:pl-0 md:pt-4">
                <span className="text-sm font-medium text-primary">Step 1</span>
                <span className="text-xl font-semibold">
                  Sign up for an account
                </span>
                <span className="mt-2 text-foreground/70">
                  Either starting out with a basic plan or choose our{" "}
                  <Link
                    href="/membership-plans"
                    className="text-primary underline underline-offset-2"
                  >
                    Pro plan
                  </Link>
                  .
                </span>
              </div>
            </li>
            <li className="md:flex-1">
              <div className="flex flex-col space-y-2 border-l-4 border-zinc-300 py-2 pl-4 md:border-l-0 md:border-t-2 md:pb-0 md:pl-0 md:pt-4">
                <span className="text-sm font-medium text-primary">Step 2</span>
                <span className="text-xl font-semibold">
                  Customize your fitness routine.
                </span>
                <span className="mt-2 text-foreground/70">
                  Dive into your gym workouts; we&apos;ve got your back!
                </span>
              </div>
            </li>
            <li className="md:flex-1">
              <div className="flex flex-col space-y-2 border-l-4 border-zinc-300 py-2 pl-4 md:border-l-0 md:border-t-2 md:pb-0 md:pl-0 md:pt-4">
                <span className="text-sm font-medium text-primary">Step 3</span>
                <span className="text-xl font-semibold">
                  Enter the gym, ignite your fitness journey.
                </span>
                <span className="mt-2 text-foreground/70">
                  Transform your body with each workout
                </span>
              </div>
            </li>
          </ol>

          <div className="mx-auto max-w-6xl px-6 lg:px-8">
            <div className="mt-16 flow-root sm:mt-24">
              <div className="-m-2 rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4">
                <Image
                  src="/images/gym_2.jpg"
                  alt="uploading preview"
                  width={1419}
                  height={732}
                  quality={100}
                  className="rounded-md bg-white p-2 sm:p-8 md:p-20 shadow-2xl ring-1 ring-gray-900/10"
                />
              </div>
            </div>
          </div>
        </div>
      </MaxWidthWrapper>
    </main>
  );
}
